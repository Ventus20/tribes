...
//switch team:		6
   ...
   switch( %id )
   {
	  ...
	  case 12:
	     doForbid(%this.player.clientId);
    
      case 24:  // SuperAdmin
         MessageBoxYesNo( "CONFIRM", "Are you sure you want to make " @ %this.player.name @ " a Super admin?",
               "lobbyPlayerVote( VoteSuperAdminPlayer, \"SA Player\", " @ %this.player.clientId @ " );" );
   }
...
//------------------------------------------------------------------------------
function doForbid(%Fclient){
   commandToServer('doForbid',%Fclient);
}
//------------------------------------------------------------------------------
