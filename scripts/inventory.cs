...
//		error("ShapeBase::use " @ %data);
       ...
       //end of modifier modes
		...
		if (%this.UsingEditorGun == 1) {
           if(%this.client.ManipulatingPiece) {
              %this.client.ManipulatingPiece = 0;
              %this.client.PieceEditing.BeingEdited = 0;
              %this.client.PieceEditing = "";
              %this.GhostPiece = "";
              commandToClient( %this.client, 'BottomPrint', "<spush><font:Tempus Sans ITC:22><color:ff3300>Editor Gun\n <font:Tempus Sans ITC:16>Piece Edit Canceled<spop>", 1, 3);
              return;
           }
		}
	...
	if (%data $= "Mine") {
		...
		if (%this.UsingEditorGun == 1) {                        // Targeting Laser
            ...
            if(!%this.client.ManipulatingPiece) {
               %this.EditingGunMode++;
			   if (%this.EditingGunMode > 1) {
				   %this.EditingGunMode = 0;
                   %this.EditingGunSnap = 0; //reset this too
			   }
               GetEditingMode(%this);
            }
            else {
               if(%this.EditingGunMode == 1) {
                  %this.EditingGunSnap++;
			      if (%this.EditingGunSnap > 3) {
				      %this.EditingGunSnap = 0;
			      }
               }
               else {
                  %this.EditingGunSnap++;
			      if (%this.EditingGunSnap > 2) {
				      %this.EditingGunSnap = 0;
			      }
               }
            }
