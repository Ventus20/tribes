...
function saveBuilding(%cl,%rad,%file,%quiet,%isAutoSave) {
	...
	$SaveBuilding::Saved[%file] = 0;
	$SaveBuilding::Skipped[%file] = 0;
	...
	while((%obj = containerSearchNext()) != 0) {
		%cmp = writeBuildingComponent(%obj,%file,0);
		if (%cmp !$= "") {
	...
	if (!$SaveBuilding::Quiet) {
		...
		warn("Saved pieces: " @ $SaveBuilding::Saved[%file]);
		warn("Skipped pieces: " @ $SaveBuilding::Skipped[%file]);
	}
...
function ResetSave(%cl) {
$SaveTime::TimeLeft[%cl.guid, "Save"]--;
...
function ResetLoad(%cl) {
$SaveTime::TimeLeft[%cl.guid, "Load"]--;
...
function PERSloadBuilding(%file) {
	compile(%file); // Just in case it bombs out
	exec(%file);
    globalPowerCheck();
}

function PersonalsaveBuilding(%cl,%rad,%file,%quiet) {
	if (%quiet == true) {
		$SaveBuilding::Quiet = true;
	}
	else {
	...
	Building.openForWrite(%file);
	Building.writeLine("// C.S.S CONSTRUCTION MOD SAVE FILE");
	Building.writeLine("// Saved by \"" @ getField(%cl.nameBase,0) @ "\"");
	Building.writeLine("// Created in mission \"" @ $MissionName @ "\"");
	Building.writeLine("// Construction " @ $ModVersion);
	Building.writeLine("");
	...
	initContainerRadiusSearch(%pos,%rad,$TypeMasks::StaticShapeObjectType | $TypeMasks::ForceFieldObjectType | $TypeMasks::ItemObjectType);
	while((%obj = containerSearchNext()) != 0) {
		%cmp = writeBuildingComponent(%obj,%file, %cl); //save the client's Pieces
		if (%cmp !$= "") {
			Building.writeLine(%cmp);
		}
	}
...
function writeBuildingComponent(%obj,%file, %cl) {
	%dataBlockName = %obj.getDatablock().getName();
	%save = false;
    if(%cl $= "" || !%cl) {
	   %save = saveBuildingCheck(%obj); //fixes the saving of invalid DBs
    }
    else {
       if(%obj.ownerGUID != %cl.GUID) {
          %save = false;
       }
       else if(%obj.ownerGUID == %cl.GUID) {
          %save = true;
       }
    }
	if (%save == true) {
		...
		if (%obj.fieldMode !$= "") %buildingPiece = %buildingPiece @ "fieldMode = \"" @ %obj.fieldMode @ "\";";
  		if (%obj.pLogic !$= "") %buildingPiece = %buildingPiece @ "pLogic = \"" @ %obj.pLogic @ "\";";
  		if (%obj.pLogic !$= "") %buildingPiece = %buildingPiece @ "pLogic = \"" @ %obj.pLogic @ "\";";
  		if (%obj.cLogic !$= "") %buildingPiece = %buildingPiece @ "cLogic = \"" @ %obj.cLogic @ "\";";
  		if (%obj.lLogic !$= "") %buildingPiece = %buildingPiece @ "lLogic = \"" @ %obj.lLogic @ "\";";
  		if (%obj.audioBlock !$= "") %buildingPiece = %buildingPiece @ "audioBlock = \"" @ %obj.audioBlock @ "\";";
