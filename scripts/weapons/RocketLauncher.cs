...
datablock ShapeBaseImageData(AALauncherImage)
{
   ...
   RankReqName = "Vehicle Destroyer"; //Called By TWMFuncitons.cs & Weapons.cs
...
function AAMissile::onExplode(%data, %proj, %pos, %mod) {
   Parent::onExplode(%data, %proj, %pos, %mod);
   %Source = %proj.sourceObject;
   %Client = %Source.client;
   if($Buying::VBoom[%Client.GUID]) {
//      echo("V-boom");
	  for(%i = 0; %i < 7; %i++){
         %p = new (TracerProjectile)(){
            dataBlock        = Flak_Bullet;
            initialDirection = "0 0 0";
            initialPosition  = %pos;
            sourceObject     = %proj; //was %obj.sourceObject
            damageFactor	 = 1;
         };
      }
   }
...
datablock ShapeBaseImageData(LMissileLauncherImage)
{
   ...
   RankReqName = "AT6 Rocket Launcher"; //Called By TWMFuncitons.cs & Weapons.cs
...
{ 
   if(!isObject(%p))
	return;
   InitContainerRadiusSearch(%p.getPosition(), 500, $TypeMasks::VehicleObjectType | $TypeMasks::PlayerObjectType);
   %searchResult = containersearchnext();
   if(%searchResult)
   {
	%SearchObj = FirstWord(%SearchResult);
    if(%SearchObj.team == %p.sourceObject.team) {

    }
    else {
       if(%SearchObj == %p.sourceObject)
          continue;
